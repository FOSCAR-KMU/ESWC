...
int traffic_light(unsigned char* srcBuf, int iw, int ih, unsigned char* outBuf, int nw, int nh, int centerP[]){
  ...
  cvtColor(yellowBinaryImg, yellowBinaryImg, CV_GRAY2BGR);
  resize(yellowBinaryImg, dstRGB, Size(nw, nh), 0, 0, CV_INTER_LINEAR);

  Point yellowCenter(-1, -1);

  get_center_point(yellowBinaryImg, & yellowCenter);


  centerP[0] = yellowCenter.x;
  centerP[1] = yellowCenter.y;


  float x_Difference = 160.0 - yellowCenter.x;
  float steer;
  int angle;

  if(x_Difference > 0.0){
    steer = 1520.0 + 3.15 * x_Difference;
  }
