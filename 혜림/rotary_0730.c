...
#define ZERO_STEERING 1500
#define MIN_STEERING 1000

#define DIST_MAX_FRONT 500
#define DIST_MAX_BEHIND 500
...
/**********************************************************************/
 ...
 bool isRotary()
 {
   ...
   if(rotary_flag >= 3)
   {
     ...
     speed = 0;
     DesireSpeed_Write(speed); // speed change
   }
 }

/*
bool checkLeftSide() // 서보모터 돌려서 왼쪽 30도 이내의 차량 확인
{
  angleX = 1200, angleY = 1500;
  CameraXServoControl_Write(angleX);
  CameraYServoControl_Write(angleY);
  //움직이는 장애물 있는지 확인 (영처)
  if()
    return true;
  //확인 후 다시 카메라 차선인식 모드
  angleX = 1500, angleY = 1200;
  CameraXServoControl_Write(angleX);
  CameraYServoControl_Write(angleY);
  return false;
}*/

bool isExistCar(struct vpe *origin)
{
  int checkX = -1, checkY = -1; // 그림자 시작좌표
  struct vpe *tmpImage; // x : 320, y : 180
  tmpImage = OpenCV_create_image(OpenCV_get_size(origin),IPL_DEPTH_8U,1);

  cvCvtColor(origin, tmpImage, CV_RGB2GRAY);
  cvNot(tmpImage, tmpImage);
  cvThreshold(tmpImage, tmpImage, 220, 255, CV_THRESH_BINARY); //threshold = 220
...
void goForward() // 직진 구간
{

}

void turnCurve() // 곡선 구간
{
...
bool checkBehindCar()
{
  int channel = 4; // behind sensor
  distance_behind = DistanceSensor(channel);
  if(distance_behind < DIST_MAX_BEHIND) // 후방에 차 접근
    return true;
  return false;
}
