...
static void drive(struct display *disp, struct buffer *cambuf)
{
    ...
    if(get_framebuf(cambuf, cam_pbuf) == 0) {
        ...
        if(rotary_flag == 2){ // 교차로 주행 중 steering값
          ...
          angle = line_detector(srcbuf, VPE_OUTPUT_W, VPE_OUTPUT_H, cam_pbuf[0], VPE_OUTPUT_W, VPE_OUTPUT_H);
        }
...
void tunnelRun()
{
  ...
  volt_left = data_transform(data2, 0 , 4095 , 0 , 5000);
...
////////////////////////////터널 변수////////////////////////////////////////
  ...
  const int max_tunnel_threshold = 1500;
