...
const Vec3b YUV_LOWER_ROTARY = Vec3b(10, 110, 120);
const Vec3b YUV_UPPER_ROTARY = Vec3b(80, 130, 140);

const Vec3b YUV_LOWER_TUNNEL = Vec3b(10, 110, 120);
const Vec3b YUV_UPPER_TUNNEL = Vec3b(70, 130, 140);
...
int enter_the_rotary(unsigned char* srcBuf, int iw, int ih, unsigned char* outBuf, int nw, int nh){
  ...
  inRange(yuvImg, YUV_LOWER_ROTARY, YUV_UPPER_ROTARY, binaryImg);
...
int is_the_tunnel(unsigned char* srcBuf, int iw, int ih, unsigned char* outBuf, int nw, int nh){
  ...
  roiImg = srcRGB(Rect(0,  srcRGB.rows/10 * 9 ,  srcRGB.cols, srcRGB.rows/10));

  cvtColor(roiImg, yuvImg, CV_BGR2YUV);

  inRange(yuvImg, YUV_LOWER_TUNNEL, YUV_UPPER_TUNNEL, binaryImg);

  for(int i = 0; i < binaryImg.cols; i++){
    for(int j = 0; j < binaryImg.rows; j++){
      if(binaryImg.at<uchar>(j, i) == 255) cnt ++;
      if(cnt > 1500) return cnt;
    }
  }
