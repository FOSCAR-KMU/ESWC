...
static void drive(struct display *disp, struct buffer *cambuf)
{
    ...
    if(get_framebuf(cambuf, cam_pbuf) == 0) {
        ...
        if(mode == 7 && passing_lane_flag == 0){
          ...
          else if(mode == 3 && temp_angle > 1850){
            temp_angle = 1520;
          }
        }
...
void mode_parking(){
  ...
  if(curve_count == 0 && angle < 1100){
    curve_count++;
    return;
  }
  else if(curve_count == 1 && angle > 1900){
    curve_count++;
    return;
  }
  else if(curve_count == 2 && angle < 1100){
    curve_count++;
