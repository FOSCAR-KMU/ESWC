...
void tunnel_run()
{
  ...
  if(dist_left < 20 && dist_right > 20) { // 왼쪽만 인식할 때
    interval = dist_left;
  ...
  else if(dist_left > 20 && dist_right < 20) { // 오른쪽만 인식할 때
    interval = dist_right;
    if(interval > 20) interval = 20;
    interval = data_transform(interval, 0, 20, -500, 500);
    temp_angle = 1520 - interval;
...
void mode_tunnel()
{
  ...
  if(control_Speed_flag == 0 && temp_angle == 1520){
    DesireSpeed_Write(80);
    control_Speed_flag = 1;
    CarLight_Write(0x01);

  }
  else if((tunnel_flag == 0) && (dist_left < start_tunnel_threshold) && (dist_right < start_tunnel_threshold)) { // 터널 시작
    tunnel_flag = 1;
    //150
