...
void go_backward()            // 주차 공간인지 판단 후 후진
{
  ...
  while(1) {
    ...
    if(dist < 70 && backward_right_flag == 0)  backward_right_flag = 1; //주차공간을 찾아서 오른쪽 조향으로 들어감
    ...
    else if(dist > 100 && backward_right_flag == 1)  backward_right_flag = 2;
  }
}
...
{
  dist = get_distance(5);
  if(dist > 20) return 0;         // 공간이 20보다 크면 수평 주차 시작
...
void return_lane_horizontal()   // 수평 주차 완료 후 차선 복귀
{
  ...
  while(1){
    dist = get_distance(2);
    if(dist > 40) break;   // 오른쪽 거리가 100 cm 이상이면 멈춤
  }
...
bool is_parking_finish()
{
  dist = get_distance(3);   //오른쪽 뒤
  dist_right = get_distance(2); //오른쪽 앞
  if(dist > 50 && dist_right > 50) return true;
  return false;
}
...
void mode_parking(){
  ...
  else if(curve_count < 3)  return; // 곡선구간 끝나기 전 
  ...
  if(parking_flag == 0 && is_parking_area()) {
  ...
  else if(parking_flag == 1 && !is_parking_area()) parking_flag = 2;  // 주차공간이 아닐 경우
  else if(parking_flag == 2 && is_parking_area()) parking_flag = 3;   // 주차공간일 경우 
  else if(parking_flag == 3)    go_backward();     // 주차 공간 인식 후 후진 시작
    ...
    if (parking_finish) DesireSpeed_Write(40); // 주차가 2번 모두 끝났을 경우 회전교차로 들어가기 전 속도 낮춤
    else  DesireSpeed_Write(80);    //주차가 끝나기 전

    while(!is_parking_finish()) {
        driving_write_steer();
    }

    if(!parking_finish) {         // 주차 1번 마친 후 
...
bool rotary_finish()
{
  if(stop_line_count > max_stop_line_threshold && rotary_finish_flag == 0) rotary_finish_flag = 1;
  else if(stop_line_count < min_stop_line_threshold && rotary_finish_flag == 1) return true;     //차가 지나간 후에 주행 시작
  return false;
}
...
bool isAnotherCar()
{
  ...
  else if(dist >= MAX_DIST) return false;
}
...
{
  // printf("MODE TUNNEL !! \n");
  dist_left = get_distance(6);  // 왼쪽 앞
  dist = get_distance(5); // 왼쪽 뒤
  ...
  else if((tunnel_flag == 0) && (dist_left < start_tunnel_threshold) && dist < start_tunnel_threshold) { // 터널 시작
    tunnel_flag = 1;
    speed = 120;
    DesireSpeed_Write(speed);
    // printf("tunnel_start!!!!\n");
...
void mode_passing_lane()
{
  // printf("%d\n", passing_lane_flag);
  // printf("%d\n", is_return);
    if(passing_lane_flag == 0)    passing_lane_flag = is_passing_lane();
	else if(passing_lane_flag == 1){}  // printf("...판별중...\n");
    ...
    else if(passing_lane_flag == 4)   return_from_right_lane();
    else if(passing_lane_flag == 5)   return_from_left_lane();
...
void left_rotate(){
  ...
  if(check_yellow_line){
    DesireSpeed_Write(-50);
    temp = data_transformF(ab[0], -0.11, 0.11,  1700000.0, 0.0);
    usleep((int) temp);

    DesireSpeed_Write(50);
    steer = data_transformF(ab[0], -0.11, 0.11, 500.0, 450.0);
    ...
    while(1){
      ...
      else if(return_flag == 1 && dist > 40)   break;
    }
    Winker_Write(ALL_OFF);
...
void right_rotate(){
  ...
  if(check_yellow_line){
    DesireSpeed_Write(-50);
    temp = data_transformF(ab[0], -0.11, 0.11,  0.0, 1700000.0);
    usleep((int) temp);
    DesireSpeed_Write(50);
    steer = data_transformF(ab[0], -0.11, 0.11, 450.0, 500.0);
