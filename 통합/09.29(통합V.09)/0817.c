...
void go_backward_right()            // 주차 공간인지 판단 후 후진
{
  ...
  while(1) {
    ...
    else if(dist > 80 && backward_right_flag == 1){
...
void left_rotate(){
  ...
  if(check_yellow_line){
    ...
    float temp = data_transformF(ab[0], -0.11, 0.11,  1500000.0, 200000.0);
    usleep((int) temp);
    DesireSpeed_Write(50);

    float steer = data_transformF(ab[0], -0.11, 0.11, 500.0, 400.0);
...
void right_rotate(){
  if(check_yellow_line){
    ...
    float temp = data_transformF(ab[0], -0.11, 0.11,  200000.0, 1500000.0);

    usleep((int) temp);
    DesireSpeed_Write(50);

    float steer = data_transformF(ab[0], -0.11, 0.11, 400.0, 500.0);
