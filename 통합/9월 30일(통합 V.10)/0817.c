...
const int start_tunnel_threshold = 15;
...
static void drive(struct display *disp, struct buffer *cambuf)
{
    ...
    if(get_framebuf(cambuf, cam_pbuf) == 0) {
        ...
        else if((mode == 5 && rotary_flag == 2) || (mode == 6 && tunnel_flag == 0)){
...
void left_rotate(){
  ...
  if(check_yellow_line){
    ...
    float temp = data_transformF(ab[0], -0.11, 0.11,  1700000.0, 200000.0);
    usleep((int) temp);
    DesireSpeed_Write(50);
...
void right_rotate(){
  ...
  if(check_yellow_line){
    ...
    float temp = data_transformF(ab[0], -0.11, 0.11,  200000.0, 1700000.0);
