...
void *capture_thread(void *arg)
{
  ...
  while (1)
  {
    ...
    case 4: // 주차
      driveOnOff = 1;
      if (parking_flag == 3 || parking_flag == 4)
        driveOnOff = 0;
...
bool start_condition()
{
  ...
  if (start_flag == 0 && dist < 10)
    start_flag = 1; // 출발전이고 손을 갖다대면 flag 1로
  else if (start_flag == 1 && dist > 20)
    start_flag = 2; // 손을 치웠을 때 flag 2로
  else if (start_flag == 2)
    return true; // 손을 치우면 출발
...
void return_lane_vertical_right() // 수직 주차 완료 후 차선 복귀
{
  ...
  while (1)
  {
    ...
    if (dist < 20)
    { //4번 센서랑 벽까지 거리가 20미만 이면 가운데 맞추면서 나옴
      angle = 1520;
      continue;
    }
...
bool rotary_finish()
{
  ...
  else if (stop_line_count < min_stop_line_threshold && rotary_finish_flag == 1)
  { //차가 지나간 후에 주행 시작
...
void mode_rotary()
{
  ...
  else if (rotary_flag == 1)
  {
    ...
    if (rotary_enter_count > max_rotary_threshold && rotary_ready_flag == 0)
    { //차가 완전히 지나가기 전
      rotary_ready_flag = 1;
    }

    else if (rotary_enter_count < min_rotary_threshold && rotary_ready_flag == 1)
    { //차가 지나간 후에 주행 시작
      printf("rotaty_start!!!");
      CameraYServoControl_Write(1680);
  ...
  { // 회전교차로 진행중
    if (isAnotherCar())
    { // 교차로 주행 중 뒤 차 유무 확인 후 속도 upupupup
      //120
  ...
  {
    if (!isAnotherCar())
    { // 뒤에 차 오는지 확인하고 사라지면 교차로 끝내고 터널 모드
      mode++;
...
void mode_tunnel()
{
  ...
  else if ((tunnel_flag == 0) && (dist_left < start_tunnel_threshold) && dist_back_left < start_tunnel_threshold)
  { // 터널 시작
    tunnel_flag = 1;
    //150
  ...
  else if ((tunnel_flag == 1) && (dist_left > finish_tunnel_threshold) && (dist_back_left > finish_tunnel_threshold))
  { // 터널 탈출
    tunnel_flag = 2;
    printf("tunnel_end!!!\n");
