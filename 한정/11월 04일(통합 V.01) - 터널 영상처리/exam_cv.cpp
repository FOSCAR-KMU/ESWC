  ...
  int passing_lane_check(unsigned char *srcBuf, int iw, int ih, unsigned char *outBuf, int nw, int nh, int temp[], float temp2[])
  {
    ...
    if (!left_error)
    {
      ...
      for (int i = 0; i < binaryImg.rows; i++)
      {
        ...
        {
          if (binaryImg.at<uchar>(i, j) == 255 && a * j + b > (float)i)
            count[0]++;
        }
      }
      temp2[0] = a;
  ...
  int traffic_light(unsigned char *srcBuf, int iw, int ih, unsigned char *outBuf, int nw, int nh, int centerP[])
  {
    ...
    if (check[0] && check[1])
    {
      ...
      if (greenCenter.x == 0 || greenCenter.y == 0)
        return -4; // 예
      //초록이 검출되면 rydiff에 빨간불과 노란불의 거리를 저장한다.
...
bool hough_left(Mat &img, Mat &srcRGB, Point *p1, Point *p2)
{
  ...
  for (int i = 10; i > 0; i--)
  {
    ...
    for (size_t j = 0; j < linesL.size(); j++)
    {
      ...
      if (CV_PI / 18 >= theta || theta >= CV_PI / 18 * 8)
        continue;
...
bool hough_right(Mat &img, Mat &srcRGB, Point *p1, Point *p2)
{
  ...
  for (int i = 10; i > 0; i--)
  {
    ...
    for (size_t j = 0; j < linesR.size(); j++)
    {
      ...
      if (CV_PI / 18 * 10 >= theta || theta >= CV_PI / 18 * 17)
        continue;
...
//binaryImg의 픽셀 중심점을 *p에 넣는다.
void get_center_point(Mat &binaryImg, Point *p)
{
  int cnt_x = 0, cnt_y = 0, count = 0;
