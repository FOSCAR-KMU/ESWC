...
//  printf("Read Encoder Counter\n");
    ...
    checksum = read_buf[0] + read_buf[1] + read_buf[2] + read_buf[3] + read_buf[4] + read_buf[5];
    if (checksum == read_buf[6])
    { 
        return ((signed int)(read_buf[5]<<24) + (signed int)(read_buf[4]<<16) + (signed int)(read_buf[3]<<8) + (signed int)(read_buf[2]));
    }
    else
    {
        //printf("!!! EncoderCounter_Read CHECKSUM ERROR %d, %d !!!\n", checksum, (int)read_buf[6]);
		puts("EncoderCounter_Read CHECKSUM ERROR");
